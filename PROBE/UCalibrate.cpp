	...
	try{
		...
		for(it = _v12.begin(); it != _v12.end(); it++){
			if(it->second.first != -1){
				std::pair<i32, i32>   item(it->first, it->second.first);
				_v12l.insert(item);
			}
			if(it->second.second != -1){
				std::pair<i32, i32>   item(it->first, it->second.second);
				_v12h.insert(item);
			}
		}
...
//-----------------------------
vector<pair<i32, i32> > __fastcall Calibrate::get_vector(u8 mode){

	std::map<i32, i32 >*	ptr = NULL;
	switch(mode){
		case 0: ptr = &_v12l;	break;
		case 1: ptr = &_v12h;	break;
		case 2: ptr = &_v27l;	break;
		case 3: ptr = &_v27l;	break;
	}
	std::map<i32, i32 >::iterator	it;

	vector<pair<i32, i32> > res;
	for(it = ptr->begin(); it != ptr->end(); it++){
		pair<i32, i32> item(it->first, it->second);
		res.push_back(item);
	}
	return res;
};
