	...
	try{
		...
		if(count == 1){
			...
			if(lua_isnumber(lvm,1)){
				...
				access_event(res);
				if(res){
//					console<<"-- Нижний уровень компарирования: " + IntToStr(a)<<endl;
				}else{
//					console<<"Ошибка обращения к устройству"<<endl;
				}
			}else{
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции low_code()"<<endl;
				stop	= true;
			}

		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции low_code()"<<endl;
			stop	= true;
		}
	...
	try{
		...
		if(count == 1){
			...
			if(lua_isnumber(lvm,1)){
				...
				access_event(res);
				if(res){
//					console<<"-- Верхний уровень компарирования: " + IntToStr(a)<<endl;
				}else{
//					console<<"Ошибка обращения к устройству"<<endl;
				}
			}else{
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции hi_code()"<<endl;
				stop	= true;
			}

		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции hi_code()"<<endl;
			stop	= true;
		}
...
//					console<<"Ошибка обращения к устройству"<<endl;
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции set_code()"<<endl;
				stop	= true;
			}

		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции set_code()"<<endl;
			stop	= true;
		}
...
//					console<<"Ошибка обращения к устройству"<<endl;
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции get_event()"<<endl;
				stop	= true;
			}
		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции get_event()"<<endl;
			stop	= true;
		}
		...
		if(count == 1){
			...
			if(lua_isnumber(lvm,1)){
				...
				bool res		= true;
					...
					access_event(res);
				if(res){
					console<<"-- На каналах 1..4 установлено напряжение : " + FloatToStr(int(100*volt)/1000.0) + "В"<<endl;
				}else{
					console<<"Ошибка обращения к ИП"<<endl;
				}
			}else{
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции set_voltage()"<<endl;
				stop	= true;
			}
			...
			if(lua_isnumber(lvm,1) && lua_isnumber(lvm,2)){
				...
				bool   res 		= psu->set_voltage(index, volt);
					...
					access_event(res);
				if(res){
					console<<"-- На канале" + IntToStr(index) + " установлено напряжение : " + FloatToStr(volt) + "В"<<endl;
				}else{
					console<<"Ошибка обращения к ИП"<<endl;
				}
			}else{
		...
		if(count == 1){
			...
			if(lua_isnumber(lvm,1)){
				...
				bool res		= true;
					...
					access_event(res);
				if(res){
					console<<"-- На каналах 1..4 установлен ток : " + FloatToStr(int(100*current)/1000.0) + "В"<<endl;
				}else{
					console<<"Ошибка обращения к ИП"<<endl;
				}
			}else{
			...
			if(lua_isnumber(lvm,1) && lua_isnumber(lvm,2)){
				...
				bool   res 		= psu->set_current(index, current);
					...
					access_event(res);
				if(res){
					console<<"-- На канале" + IntToStr(index) + " установлено напряжение : " + FloatToStr(current) + "В"<<endl;
				}else{
					console<<"Ошибка обращения к ИП"<<endl;
				}
			}else{
	...
	try{
		...
		if(count == 1){
			...
			if(lua_isnumber(lvm,1)){
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции get_voltage()"<<endl;
				stop	= true;
			}
		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции get_voltage()"<<endl;
			stop	= true;
		}
	...
	try{
		...
		if(count == 1){
			...
			if(lua_isnumber(lvm,1)){
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции get_current()"<<endl;
				stop	= true;
			}
		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции get_current()"<<endl;
			stop	= true;
		}
...
int 	create(lua_State*	lvm){
	...
	try{
		...
		console<<"Таблица кроссировки проинициализирована"<<endl;
...
int 	close(lua_State*	lvm){
	...
	try{
		...
		console<<"Таблица кроссировки подготовлена для сохранения"<<endl;
	...
	try{
		...
		if(count == 3){
			...
			if(lua_isnumber(lvm,1) && lua_isnumber(lvm,2) && lua_isnumber(lvm,3)){
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции add_12v()"<<endl;
				stop	= true;
			}

		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции add_12v()"<<endl;
			stop	= true;
		}
	...
	try{
		...
		if(count == 3){
			...
			if(lua_isnumber(lvm,1) && lua_isnumber(lvm,2) && lua_isnumber(lvm,3)){
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции add_27v()"<<endl;
				stop	= true;
			}

		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции add_27v()"<<endl;
			stop	= true;
		}
	...
	try{
		...
		if(count == 1){
			...
			if(lua_isnumber(lvm,1)){
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции to_code_12v()"<<endl;
				stop	= true;
			}
		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции to_code_12v()"<<endl;
			stop	= true;
		}
	...
	try{
		...
		if(count == 1){
			...
			if(lua_isnumber(lvm,1)){
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции to_code_27v()"<<endl;
				stop	= true;
			}
		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции to_code_27v()"<<endl;
			stop	= true;
		}
	...
	try{
		...
		if(count == 0){
			...
			if(lua_isstring(lvm,1)){
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции save()"<<endl;
				stop	= true;
			}
		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции save()"<<endl;
			stop	= true;
		}
		...
		if(count == 2){
			...
			if(lua_isnumber(lvm,1) && lua_isnumber(lvm,2)){
				...
				bool   res 			= mgqs->set_amplitude(index, amplitude);
					...
					access_event(res);
				if(res){
					console<<"-- На канале" + IntToStr(index) + " установлена амплитуда : " + FloatToStr(amplitude) + "В"<<endl;
				}else{
					console<<"Ошибка обращения к МГКС"<<endl;
				}
			}else{
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции set_amplitude()"<<endl;
				stop	= true;
			}

		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции set_amplitude()"<<endl;
			stop	= true;
		}
		...
		if(count == 2){
			...
			if(lua_isnumber(lvm,1) && lua_isnumber(lvm,2)){
				...
				bool   res 			= mgqs->set_offset(index, offset);
					...
					access_event(res);
				if(res){
					console<<"-- На канале" + IntToStr(index) + " установлено смещение: " + FloatToStr(offset) + "В"<<endl;
				}else{
					console<<"Ошибка обращения к МГКС"<<endl;
				}
			}else{
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции set_offset()"<<endl;
				stop	= true;
			}

		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции set_offset()"<<endl;
			stop	= true;
		}
	...
	try{
		...
		if(count == 0){
			...
			if(lua_isnumber(lvm,1)){
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции mgqs_enable()"<<endl;
				stop	= true;
			}
		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции mgqs_disable()"<<endl;
			stop	= true;
		}
 	...
 	try{
		...
		if(count == 0){
			...
			if(lua_isnumber(lvm,1)){
				...
				console<<"-- Error: Типы параметров не соответствует сигнатуре функции mgqs_disable()"<<endl;
				stop	= true;
			}
		}else{
			console<<"Error: Количество параметров не соответствует сигнатуре функции mgqs_disable()"<<endl;
