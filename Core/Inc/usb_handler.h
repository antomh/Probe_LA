...
typedef struct {
    /* Флаг is_handled нужен для отслеживания факта обработки пакета, чтобы не
     * произошла коллизия между пакетами. Пока этот флаг поднят информация не
...
/*  Command system
    ...
    0х0D - Запись во флеш калибровочной таблицы     data: 0B
                                                    answer: 0x0D + 1B status
    ...
    0x0F - Считывание калибровочной таблицы из flash-памяти щупа
                                                    data: [0x0F]
                                                    answer: [0x0F][1-4][start number][end number][data]
    ...
    --------------------------------------------------------------------------
    Калибровка
    На вход щупа подается симметричная пила с частотой 1кГц с оффсетом установленным на ип.
    после компарирования сигнала МК измеряет длительность импульса
    т.к. 1 кГц соотвествет длительности в 500 мс то это означает что мы попапали в полку офсета. */
/*---------------------------------------------------------------------------*/
